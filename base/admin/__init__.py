from .about_us import AboutUsAdmin
from .contact_us import ContactUsDetailAdmin, ContactUsFormAdmin
from .dynamic_text import DynamicTextAdmin
from .faq import FAQAdmin
from .footer import FooterAdmin
from .menu import MenuAdmin
from .object_instance import ObjectInstanceAdmin
from .page import PageAdmin
from .singleton import SingletonModelAdmin
from .slider import SliderAdmin
from .social_account import SocialAccountsAdmin
from .state import CityAdmin, StateAdmin
from .terms_and_conditions import TermsAndConditionsAdmin
