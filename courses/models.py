...
class Course(BaseModel):
    ...
    category = models.ForeignKey(
        to=CourseCategory, verbose_name=_("category"), on_delete=models.CASCADE
    )
    image = models.FileField(verbose_name=_("image"), upload_to="course-images")
    image_alt = models.CharField(max_length=200, null=True, blank=True)
    video = models.FileField(
        verbose_name=_("video"), upload_to="course-videos", null=True, blank=True
    )
    cover = models.FileField(
        verbose_name=_("cover"), upload_to="course-covers", null=True, blank=True
    )
    ...
    def full_access_user(self, user) -> bool:
        lessons = self.lesson_set.all().values_list("id", flat=True)
        chapters = Lesson.objects.filter(id__in=lessons).values_list(
            "chapter", flat=True
        )
        notes = Chapter.objects.filter(id__in=chapters).values_list("note", flat=True)
        if set(notes) == set(
            Note.objects.filter(participants=user, chapter__in=chapters).values_list(
                "id", flat=True
            )
        ):
            return True
...
class Lesson(BaseModel):
    course = models.ForeignKey(
        to=Course, verbose_name=_("course"), on_delete=models.CASCADE
    )
    title = models.CharField(max_length=200, verbose_name=_("title"))
    ...
    def full_access_user(self, user) -> bool:
        chapters = self.chapter_set.all().values_list("id", flat=True)
        notes = Chapter.objects.filter(id__in=chapters).values_list("note", flat=True)
        if set(notes) == set(
            Note.objects.filter(participants=user, chapter__in=chapters).values_list(
                "id", flat=True
...
class Chapter(BaseModel):
    lesson = models.ForeignKey(
        to=Lesson, verbose_name=_("lesson"), on_delete=models.CASCADE
    )
    title = models.CharField(max_length=200, verbose_name=_("title"))
    ...
    def full_access_user(self, user) -> bool:
        if set(self.note_set.all().values_list("id", flat=True)) == set(
            Note.objects.filter(participants=user, chapter=self).values_list(
                "id", flat=True
            )
        ):
            return True
...
class Note(BaseModel):
    chapter = models.ForeignKey(
        to=Chapter, verbose_name=_("chapter"), on_delete=models.CASCADE
    )
    title = models.CharField(max_length=200, verbose_name=_("title"))
    price = models.PositiveIntegerField(verbose_name=_("price"))
...
class NoteTaking(models.Model):
    note = models.ForeignKey(to=Note, on_delete=models.CASCADE, verbose_name=_("note"))
    user = models.ForeignKey(
        to="accounts.User", on_delete=models.CASCADE, verbose_name=_("user")
    )
    content = RichTextUploadingField(verbose_name=_("content"))
