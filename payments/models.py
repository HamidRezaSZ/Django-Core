...
class PaymentGateWay(BaseModel):
    ...
    request_url = models.CharField(max_length=500, verbose_name=_("request_url"))
    verify_url = models.CharField(max_length=500, verbose_name=_("verify_url"))
    startpay_url = models.CharField(max_length=500, verbose_name=_("startpay_url"))
...
class Payment(models.Model):
    ...
    ref_id = models.CharField(
        verbose_name=_("ref_id"), max_length=512, blank=True, null=True
    )
    status = models.ForeignKey(
        to=PaymentStatus, on_delete=models.CASCADE, verbose_name=_("status")
    )
    authority = models.CharField(
        verbose_name=_("authority"), max_length=512, blank=True, null=True
    )
    link = models.URLField(verbose_name=_("link"), blank=True, null=True)
    payment_gateway = models.ForeignKey(
        to=PaymentGateWay, verbose_name=_("payment_gateway"), on_delete=models.PROTECT
    )
    ...
    def save(self, *args, **kwargs) -> None:
        if not self.pk:
            ...
            status, created = PaymentStatus.objects.get_or_create(status="Pending")
            self.status = status
            try:
                response = requests.post(
                    self.payment_gateway.request_url,
                    data=data,
                    headers=headers,
                    timeout=10,
                )
                if response.status_code == 200:
                    ...
                    if response["Status"] == 100:
                        ...
                        self.link = self.payment_gateway.startpay_url + str(
                            response["Authority"]
                        )
                        self.status = status
                    else:
                ...
                status, created = PaymentStatus.objects.get_or_create(status="Error")
                self.status = status
            except Exception:
                status, created = PaymentStatus.objects.get_or_create(status="Error")
                self.status = status
