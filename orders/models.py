...
class DeliveryType(BaseModel):
    ...
    delivery_price = models.PositiveIntegerField(
        default=0, verbose_name=_("delivery_price")
    )
...
class Order(models.Model):
    ...
    address = models.ForeignKey(
        "accounts.Address", on_delete=models.PROTECT, verbose_name=_("address")
    )
    delivery_type = models.ForeignKey(
        DeliveryType, on_delete=models.PROTECT, verbose_name=_("delivery_type")
    )
    ...
    coupon_code = models.CharField(
        max_length=200, blank=True, null=True, verbose_name=_("coupon_code")
    )
    price = models.PositiveIntegerField(default=0, verbose_name=_("price"))
    discount_amount = models.PositiveIntegerField(
        default=0, verbose_name=_("discount_amount")
    )
    status = models.ForeignKey(
        to=OrderStatus, on_delete=models.CASCADE, verbose_name=_("status")
    )
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        payment_obj = Payment.objects.create(
            user=self.user, amount=self.price - self.discount_amount
        )
...
class OrderItem(models.Model):
    order = models.ForeignKey(
        to=Order, verbose_name=_("order"), on_delete=models.CASCADE
    )
    product_quantity = models.ForeignKey(
        "products.ProductQuantity",
        on_delete=models.PROTECT,
        verbose_name=_("product_quantity"),
    )
